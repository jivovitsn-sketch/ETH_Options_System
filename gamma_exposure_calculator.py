#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""GAMMA EXPOSURE CALCULATOR - ALL 6 ASSETS [использует unlimited_oi.db]"""
...
DEFAULT_IV = 0.80  # 80% дефолтная волатильность для крипты
...
class GammaExposureCalculator:
    ...
    def load_options_data(self):
        try:
            ...
            # Берём только активные опционы (OI > 0)
            query = """
                SELECT strike, expiry_date, option_type, open_interest, 
                       spot_price, dte, volume_24h
    ...
    def calculate_gamma_exposure(self):
        ...
        for _, row in self.options_data.iterrows():
            try:
                ...
                # Используем дефолтную IV (в реальной системе можно улучшить)
                sigma = DEFAULT_IV
                ...
                # PUT опционы создают отрицательную гамму для MM
                if opt_type == 'put':
                    gex = -gex
    ...
    def find_zero_gamma_level(self):
        ...
        strikes = np.array(list(self.gex_by_strike.keys()))
        gex_values = np.array(list(self.gex_by_strike.values()))
        ...
        if len(sign_changes) > 0:
            ...
            y1, y2 = gex_values[idx], gex_values[idx+1]
    ...
    def save_to_database(self):
        try:
            ...
            cursor = conn.cursor()
            cursor.execute('''CREATE TABLE IF NOT EXISTS gamma_exposure (
                id INTEGER PRIMARY KEY AUTOINCREMENT, symbol TEXT, strike REAL, 
                gex_value REAL, spot_price REAL, total_gex REAL, zero_gamma_level REAL, 
                timestamp DATETIME DEFAULT CURRENT_TIMESTAMP, 
                UNIQUE(symbol, strike, timestamp))''')
            
            timestamp = datetime.now()
            for strike, gex in self.gex_by_strike.items():
                cursor.execute('''INSERT OR REPLACE INTO gamma_exposure 
                    (symbol, strike, gex_value, spot_price, total_gex, zero_gamma_level, timestamp) 
                    VALUES (?, ?, ?, ?, ?, ?, ?)''',
                    (self.symbol, strike, gex, self.spot_price, self.total_gex, 
                     self.zero_gamma_level, timestamp))
    ...
    def plot_gamma_exposure(self):
        ...
        if self.spot_price:
            ax1.axvline(self.spot_price, color='blue', linestyle='--', linewidth=2, 
                       label=f'Spot: ${self.spot_price:,.0f}')
        if self.zero_gamma_level:
            ax1.axvline(self.zero_gamma_level, color='orange', linestyle='--', linewidth=2, 
                       label=f'Zero Gamma: ${self.zero_gamma_level:,.0f}')
        
        ax1.set_xlabel('Strike ($)', fontsize=12)
        ax1.set_ylabel('GEX ($)', fontsize=12)
        ax1.set_title(f'{self.symbol} Gamma Exposure\nTotal GEX: ${self.total_gex:,.0f}', 
        ...
        if self.spot_price:
            ax2.axvline(self.spot_price, color='blue', linestyle='--', linewidth=2)
    ...
    def export_to_json(self):
        data = {
            ...
            'spot_price': self.spot_price, 'total_gex': self.total_gex,
    ...
    def run_full_calculation(self):
        ...
        if not self.calculate_gamma_exposure():
            logger.error("❌ GEX calculation failed")
            return False
...
def main():
    print("="*80)
    print("🧮 GAMMA EXPOSURE CALCULATOR - ALL 6 ASSETS")
    print("="*80 + "\n")
    ...
    for symbol in SYMBOLS:
        try:
            ...
            results[symbol] = {
                'success': success,
                'total_gex': calc.total_gex if success else 0,
                'zero_gamma': calc.zero_gamma_level,
    ...
    print("\n" + "="*80)
    print("📈 FINAL RESULTS - ALL 6 ASSETS")
    print("="*80)
    ...
    for symbol, r in results.items():
        if r['success']:
            ...
            print(f"\n✅ {symbol}:")
            print(f"   Spot: ${r['spot_price']:,.2f} | Total GEX: ${r['total_gex']:,.0f}{zg_str}")
        else:
            print(f"\n❌ {symbol}: FAILED")
    
    print("\n" + "="*80)
    print("✅ STAGE 1.3.1 COMPLETE - ALL 6 ASSETS!")
    print("="*80 + "\n")
